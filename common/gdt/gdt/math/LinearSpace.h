...
namespace gdt {
  ...
  inline __both__ linear3f createRotationMatrix(const vec3f& axis, const float angle) {
    auto normAxis = normalize(axis);
    float x = normAxis.x;
    float y = normAxis.y;
    float z = normAxis.z;
    ...
    linear3f rotation = {
      cosTheta + x * x * (1 - cosTheta), x * y * (1 - cosTheta) - z * sinTheta, x * z * (1 - cosTheta) + y * sinTheta,
      y * x * (1 - cosTheta) + z * sinTheta, cosTheta + y * y * (1 - cosTheta), y * z * (1 - cosTheta) - x * sinTheta,
      z * x * (1 - cosTheta) - y * sinTheta, z * y * (1 - cosTheta) + x * sinTheta, cosTheta + z * z * (1 - cosTheta)
    };

    return rotation;
  }
  
  inline __both__ vec3f rotateVector(const vec3f& v, vec3f& target, linear3f &rotation, float shininess) {
    const vec3f UP = vec3f(0.f, 1.f, 0.f);
    auto orthonormal = cross(UP, target);
    float angle = acos(dot(UP, target));

    rotation = createRotationMatrix(orthonormal, angle);

    vec3f rotatedVector = rotation * v;
